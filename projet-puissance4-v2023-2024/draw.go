...
func (g game) colorSelectDraw(screen *ebiten.Image) {
	if !g.attente_second_joueur {
		text.Draw(screen, "Quelle couleur pour vos pions ?", smallFont, 110, 80, globalTextColor)
	} else {
		text.Draw(screen, "En attente de votre partenaire", smallFont, 110, 80, globalTextColor)
	}
...
func (g game) resultDraw(screen *ebiten.Image) {
	...
	if g.attente_second_joueur {
		...
		text.Draw(screen, "En attente du rematch", smallFont, 300, 400, globalTextColor)
	}
